...
def process_file(file_path: Path, output_dir: Path, table_index):
    if output_dir is None:
        output_dir = Path('.').resolve()
    raw = read_sinar(file_path)
...
class IaApp(toga.App):
    ...
    def startup(self):
        ...
        self.img_view = toga.ImageView(id='view1', image=toga.Image(path='test.jpg'))
    ...
    def print_console(self, line, end='\n'):
    ...
    async def import_folder(self, sender):
        input_path_name = await self.main_window.select_folder_dialog(
            ...
            initial_directory=Path('.').resolve()
        )
        self.print_console(f'Selected {input_path_name.name} for import...')
        ...
        if input_path_name is not None and dest_path_name is not None:
            ...
            self.print_console(f'Copying {len(file_list)} files from {input_path_name.name} to {dest_path_name.name}...')
            self.pool.apply_async(import_files_process, args=(file_list, dest_path_name),
    ...
    async def set_output(self, sender):
        ...
        if path_name is not None:
            ...
            self.print_console(f'Set {path_name.name} as output directory.')
            self.print_console(self.text_state())
    ...
    async def start_process(self, sender):
        for i, file_path in enumerate(self.file_dict.values()):
            self.pool.apply_async(process_file, (file_path, self.output_path, i),
                                  callback=self.process_done,
                                  error_callback=self.process_failed)
        self.print_console(f'Processing... {len(self.file_dict)} files.')

    def find_ia_files(self, directory):
        self.file_dict = {f.name: f for f in Path(directory).glob('*.IA')}
        self.file_table.data = [{'ia_file': f, 'processed': False} for f in self.file_dict.keys()]
        self.print_console(f'Opened {directory.name}')
        self.print_console(self.text_state())

    async def load_meta_and_thumb(self, sender, row):
        file = self.file_dict[row.ia_file]
        sinar_ia_raw = read_sinar(file)
